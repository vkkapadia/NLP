...
def load_model(
    model_path="model/llama-2-7b-chat.ggmlv3.q2_K.bin",
    model_type="llama",
    max_new_tokens=512,
    temperature=0.7,
):
    ...
    #      pipeline_kwargs={"temperature": temperature, "do_sample": True, "max_new_tokens": max_new_tokens},
    #  )
    llm = CTransformers(
        model=model_path,
...
async def process_chat_message(message):
    ...
    response = await qa_chain.ainvoke(message, callbacks=[callback_handler])
